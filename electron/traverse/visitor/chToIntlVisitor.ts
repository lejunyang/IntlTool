 ...
 * @LastEditTime: 2022-11-21 16:02:56
...
export const getChToIntlVisitor = (options: IntlOptions) => {
  ...
  const visitor: Visitor<ProcessFile> = {
    ...
    VariableDeclarator(path: NodePath<VariableDeclarator>, state) {
      ...
      state.chTransformedInfo.push({
        start: node.start,
        end: node.end,
        original: generateCode(node),
        replace: generateCode(replaceNode),
        location: `${state.path}:${node.loc?.start.line || 0}:${node.loc?.start.column || 0}`,
      });
    },
    // 赋值表达式
    ...
    JSXText(path: NodePath<JSXText>, state) {
      ...
      state.chTransformedInfo.push({
        ...
        original: value,
        replace: generateCode(replaceNode),
        location: `${state.path}:${path.node.loc?.start.line || 0}:${path.node.loc?.start.column || 0}`,
      });
    },
    // jsx表达式
    ...
    ArrayExpression(path: NodePath<ArrayExpression>, state) {
      ...
      elements.forEach((e, index) => {
        ...
        state.chTransformedInfo.push({
          ...
          original: generateCode(e),
          replace: generateCode(replaceNode),
          location: `${state.path}:${e.loc?.start.line || 0}:${e.loc?.start.column || 0}`,
        });
      });
    },
    /*
    ...
    CallExpression(path: NodePath<CallExpression>, state) {
      ...
      args.forEach((a, index) => {
        ...
        state.chTransformedInfo.push({
          ...
          original: generateCode(a),
          replace: generateCode(replaceNode),
          location: `${state.path}:${a.loc?.start.line || 0}:${a.loc?.start.column || 0}`,
        });
      });
    },
    // 针对new Error('中文')的情况
